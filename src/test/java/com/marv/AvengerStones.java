package com.marv;

public class AvengerStones {
	public void soulStone() {
		System.out.println("Hawkeye");
	}

	public void mindStone() {
		System.out.println("Vision");
	}

	public void powerStone() {
		System.out.println("Star-Lord");
	}

	public void spaceStone() {
		System.out.println("Loki");
	}

	public void timeStone() {
		System.out.println("Dr.Strange");
	}

	public void realityStone() {
		System.out.println("AsGuard");
	}

	private void wanda() {
		System.out.println("Scarlet witch");
	}

	private void loki() {
		System.out.println("God of Mischief");
	}

	private void hulk() {
		System.out.println("Bruce banner");
	}

	private void agentramonoff() {
		System.out.println("Black Widow");

	}

	private void Hawkeye() {
		System.out.println("advanced weaponry and gadgets");
		
	}
	
	private void Blackpanther() {
		System.out.println("Necromancy");
	}
	
	private void Ironman() {
		System.out.println("cyberpathic control ");
	}
	
	
}
